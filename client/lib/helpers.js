if(Meteor.isClient) {
  ...
  setNextDisplayDate = function() {
    ...
    console.log(remainingMessagesArray);

    if( !_.isEmpty(remainingMessagesArray) ) {
      var nextDisplayDate = moment(remainingMessagesArray[remainingMessagesArray.length - 1].displayDate).add(1, "days").toDate();
  ...
  setMessage = function() {
    ...
    var displayedMessages = Messages.find({ displayDate: { $lte: getTodayUTCMidnight() } });
    var displayedMessagesArray = displayedMessages.fetch();

    if( !_.isEmpty(displayedMessagesArray) ) {

      var message = displayedMessagesArray[displayedMessagesArray.length - 1];

      Session.set("message", message.text);
      Session.set("count", displayedMessagesArray.length);
      if(Meteor.isClient) {
  ...
  stylePage = function(text) {
    ...
    $("h1 span, h2 span, p, a.close, footer, footer a")
    .css("color", color2);
  ...
  getBaseSettings = function(text) {
    ...
    if(!_.isEmpty(text)) {
      ...
      var schemeTypes = ["split", "complement", "tri"];
      ...
      {
        scheme_type: schemeTypes[ Math.round( this.mapValue(similarity, 0.91, 0.99, 0, 2) ) ],
        format: 'rgb'
      });
      colorScheme.sort(function() { return 0.5 - Math.random() });

      fontStyle = "style" + Math.round( this.mapValue(similarity, 0.9175, 0.9875, 1, 55) );
    }
    else {
      opacity = 0.25;
      colorScheme = [{r: 0, g: 0, b: 0}, {r: 255, g: 255, b: 255}, {r: 0, g: 0, b: 0}];
      fontStyle = "style0";
    }
    return {
